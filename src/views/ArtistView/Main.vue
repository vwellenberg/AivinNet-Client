<template>
    <div class="artist-page v-scroll-page" style="height: 100%; position: relative;"
        :style="{ background: store.colors.bg ? `linear-gradient(180deg, ${vibrateHex(store.colors.bg)} 0%, ${vibrateHex(store.colors.bg)}CC 25%, ${store.colors.bg}55 55%, transparent 80%)` : `linear-gradient(180deg, #4a4a4a 0%, #1c1c1c 40%, transparent 70%)` }">
        <DynamicScroller
            id="artist-scroller"
            :items="scrollerItems"
            :min-item-size="64"
...
function getStatsComponent(stats: StatItem[]): ScrollerItem {
    return {
        id: Math.random(),
        component: Stats,
        props: {
...
const scrollerItems = computed(() => {
    ...
    if (store.compilations.length > 0) {
        const compilations = createAbumComponent(AlbumType.COMPILATIONS, store.compilations)
        components.push(compilations)
    }

    if (store.appearances.length > 0) {
        const appearances = createAbumComponent(AlbumType.APPEARANCES, store.appearances, false)
    ...
    if (store.stats.length > 0) {
        components.push(getStatsComponent(store.stats))
    }

    if (store.fetched_similar_hash === route.params.hash && store.similar_artists.length > 0) {
