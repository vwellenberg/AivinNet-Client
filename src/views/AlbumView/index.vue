<template>
    <div class="album-virtual-scroller v-scroll-page" :class="{ isSmall }" style="height: 100%; position: relative;"
        :style="{ background: album.colors.bg ? `linear-gradient(180deg, ${vibrateHex(album.colors.bg)} 0%, ${vibrateHex(album.colors.bg2 || album.colors.bg)}EE 20%, ${album.colors.bg}AA 42%, ${album.colors.bg}55 65%, transparent 95%)` : `linear-gradient(180deg, #4a4a4a 0%, #1c1c1c 40%, transparent 70%)` }">
        <DynamicScroller
            id="album-scroller"
            style="height: 100%"
            class="scroller"
