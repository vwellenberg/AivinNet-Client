...
- **Eine geteilte Komponente nimmt ihre Rolle SELBST** — sie von außen zu patchen ist die Drift, die #90 einsammelt. `HeartSvg.vue` trug seine Maße lange von Hand, also wiederholten Album- und Artist-Header wortgleich denselben Patch (`@include btn-action` + Wieder-Setzen des Teals). Jetzt hat die Komponente eine Prop `btn_role` (`quiet` = blanker Glyph in Bar/Zeilen/NowPlaying, `action` = Header-Platte) und die Korrektur liegt dort, wo die Kollision entsteht. Neue Varianten also als **Rollen-Prop an der Komponente**, nicht als Regel in der aufrufenden View.
- **Detail-Header haben eine feste Reihenfolge:** Play · Favorit · Pin · Zweitaktion · Overflow. Es gibt **vier** davon — Album, Artist, Playlist und **Mix** (`Mixes/MixesHeader.vue`, Route `/mix/:mixid`); der letzte wird beim Suchen leicht übersehen. Die Reihe selbst ist die geteilte Klasse **`.header-actions`** in [_button-classes.scss](src/assets/scss/Global/_button-classes.scss) (Flex + Gap + Wrap + Staffel-Versatz) — eine neue Header-Reihe bekommt einfach die Klasse, es gibt keine Liste mehr, in die man sich eintragen müsste. **Reihenfolge und Vollständigkeit sind getestet** (`src/components/__tests__/headerActionOrder.test.ts`): ein Button am falschen Ende der Reihe oder eine fünfte `.header-actions`-Reihe, die der Test nicht kennt, lässt die Suite rot laufen.
...
- **Mess-Werkzeuge in `~/uitest` (Chromium **und** Firefox installiert).** Vor dem Bauen eines neuen Skripts hier nachsehen — die meisten Fragen sind schon einmal gemessen worden:
  - `rangeshot.js` + `rangemeasure.py` — Regler-Geometrie in **Pixeln** (Knopf gegen Leiste, Mittellinien-Versatz). Der Knopf ist ein Pseudo-Element und hat keine Bounding-Box; gemessen wird deshalb im gerenderten Bild.
  - `wavecheck.js` — welches Element beim Klick eine `v-wave`-Welle wirft, mit Farbe und Dauer.
  - `popcheck.js` — liest die **laufende** Transform-Matrix aus (`getComputedStyle().transform` während die Animation läuft), statt der Keyframe-Deklaration zu glauben.
  - `btnaudit.js`, `bordermeasure.js`, `audit-shadows.js`, `mobile-audit.js` — Computed-Style-Audits über Routen × Themes.
  - `previewproxy.js` + die `run*.sh` — Branch-`dist` über einen Proxy servieren und messen. Die Runner bündeln Proxy (bg) + Messung (fg) + `kill` in **einer** SSH-Sitzung, weil ein detachter Dienst die Sitzung hängen lässt.
  - **Gegen `master` kontrollieren, immer.** Eine Null beweist ohne Kontrolllauf nur, dass man nicht misst.
...
- **⚠️ Touch-Ziele: 44 px, und Header-Aktionen haben EINE Anatomie:** Sekundäre Aktionen in Seiten-Headern (Favorit, Pin, Download, Cover holen, Optionen, Edit, Löschen) nehmen die Rolle **`btn-action`** aus [Global/_buttons.scss](src/assets/scss/Global/_buttons.scss) — 2.75rem quadratisch, Panel-Fläche, Rahmen, Offset-Schatten, 1.5rem-Glyph, `flex-shrink: 0`. (Bis v1.5.1 hieß das `mem-header-action` in `_candy.scss`; **das Mixin gibt es seit #244 nicht mehr** — wer danach sucht, sucht vergeblich.) Die beiden Fallen dahinter: (1) Ohne `flex-shrink: 0` **quetscht** eine Header-Reihe auf schmalen Phones ihre eigenen Buttons (gemessen: 16×36 statt 40×40) — Header-Reihen bekommen `flex-wrap: wrap`, und der Header eine `min-height` statt fixer `height`, sonst läuft die umgebrochene Reihe in den Inhalt darunter. (2) Transparente Buttons erben aus der globalen Button-Basis **statisches Ink** — auf dem dunklen Ground waren Pin und Download schlicht **unsichtbar**; deshalb `color: $mem-content-text`. Messen statt schätzen: `~/uitest/mobile-audit.js` listet pro Route × Theme alle interaktiven Elemente < 44 px und jedes Element breiter als der Viewport. Legitim unter 44 px bleiben nur der Slider-Track (der Knopf ist das Ziel) und Inline-Textlinks in Listenzeilen (die ganze Zeile ist tappbar).
- **⚠️ `aspect-ratio` braucht eine Dimension zum Auflösen:** `aspect-ratio: 1.5` allein ergibt nichts — es braucht Höhe **oder** Breite. Früher kam die Höhe unbemerkt aus der globalen Button-Basis (`button { height: 2.25rem }`); **seit #244 ist diese Basis ein reiner Reset** und liefert sie nicht mehr. Wer nur eine der beiden Maße setzt, bekommt ein **Oval** — bei `border-radius: 50%` eine Ellipse (real passiert: `.heart-button` fiel von 54×36 auf 28×28, und der Now-Playing-Reglerknopf war eine liegende Ellipse, weil nur seine Höhe überschrieben war). Immer beide Maße setzen oder eines explizit stehen lassen. `.heart-button` tut das inzwischen über seine Rolle (`btn-quiet($size, $width, …)`) und hat gar keine `aspect-ratio` mehr — die Rolle ist auch hier der Ort, an dem so etwas EINMAL steht.
...
- **⚠️ `:hover` latcht auf Touch — nie zum Verstecken nutzen.** Auf dem Handy bleibt der Hover-Zustand nach dem ersten Tap hängen. Die mobile Seek-Bar hatte genau das: `.b-bar:hover #progress::-webkit-slider-thumb { display: none }` — der Knopf verschwand also, sobald man ihn anfasste (dazu `pointer-events: none` auf `.center` und ein 1px-Track; die Bar war im Fork bewusst reine Deko). Für Drag-Controls gilt: sichtbarer Knopf, Touch-Größe (`range-geometry` auf dem Wrapper, siehe unten) und **`touch-action: none`** auf dem Input, sonst frisst der Page-Scroll die horizontale Geste. Dieselbe Falle in klein: eine `.b-bar:hover`-Regel verkleinerte den Knopf, sobald die Maus **irgendwo** in die Leiste kam — auf Touch also dauerhaft, nach dem ersten Tap (entfernt in #284).
...
- **⚠️ Sass wertet NICHTS innerhalb eines Custom-Property-Werts aus.** `--btn-pop-delay: $motion-stagger * 3` landet **wörtlich** im Stylesheet — der Wert ist für Sass ein undurchsichtiges Token. Kein Fehler beim Bauen, kein Lint-Befund, keine fehlschlagenden Tests: im Browser ist der Wert einfach ungültig und die `var()`-Fallback greift. Immer `#{}` interpolieren: `--x: #{$token * 3}`. Real passiert bei #240 — der ganze Staffel-Effekt aus #279 wäre still gestorben.
- **Der `animation`-Kurzform setzt `animation-delay` und `-fill-mode` zurück.** Die Button-Rollen schreiben die komplette Kurzform, und sie tun das aus einem **tieferen** Selektor, als eine Zeilen-Regel erreichen kann — ein `animation-delay` auf den Kindern einer Reihe wurde deshalb jedes Mal weggewischt (gemessen: 4 von 5 Buttons bei 0s). Werte, die eine Rolle überleben sollen, gehören in eine **Custom Property**, die die Kurzform liest (`var(--btn-pop-delay, 0s)`).
- **`animation-fill-mode: backwards` ist erlaubt, `both` nicht.** `both` hält zusätzlich den **letzten** Frame, und ein Animations-`scale(1)` schlägt jedes deklarierte `transform` — Hover und Press wären in allen fünf Rollen still tot. `backwards` gilt nur **während** einer Verzögerung und gibt das Element danach frei; es ist die Voraussetzung für gestaffelte Reihen (sonst erscheint ein verzögerter Button erst groß und schrumpft dann zum Poppen).
- **⚠️ Reine Refactors gegen den Content-Hash beweisen.** Vite benennt Assets nach ihrem Inhalt — wenn `dist/assets/index.<hash>.css` auf Branch und master **denselben** Namen trägt, ist das gebaute CSS byte-identisch und eine Rendering-Änderung ausgeschlossen, nicht bloß unwahrscheinlich. Vorgehen: beide Seiten mit `rm -rf dist` sauber bauen, Dateinamen vergleichen, mit `cmp` gegenprüfen. Genau dieser Vergleich hat den Sass-Interpolationsfehler oben gefunden, den drei grüne Gates durchgelassen hatten.
- **⚠️ Regler-Geometrie hat EINE Quelle:** `range-geometry($h, $thumb)` in `_candy.scss` setzt `--range-h` (Leistenhöhe), `--range-thumb` (Knopfdurchmesser) und das daraus abgeleitete `--range-track` (Innenhöhe). Die Zentrierung wird **berechnet**, nicht geraten — ein fester `margin-top` kann nur zu genau einem Paar aus Leiste und Knopf passen, und es gab vier. Die Variablen gehören auf den **Wrapper** (`.progress-wrap`), nicht auf den `input`: der Textur-Overlay ist ein *Geschwister* des Inputs und sieht dort gesetzte Werte nie. `ProgressBar.scss` liest nur noch, immer mit Fallback — eine Deklaration dort würde alles überschatten, was ein Host weiter oben setzt.
- **⚠️ Eine Knopf-Messung ist blind für Geschwister-Overlays.** Ein `margin-top` auf dem Regler-`input` bläht den Wrapper auf (Inline-Block: die Margin-Box zählt zur Zeilenbox), ohne die Mitte des Inputs mitzubewegen. Knopf und Leiste zentrieren auf dem Input, der Overlay auf dem Wrapper → 7 px auseinander, während jede Knopf-Messung ±0,00 px meldet. Abstände gehören auf den Wrapper.
- **Klick-Feedback-Regel:** **Zeilen** (Nav, Playlists, gepinnte Alben, Ordner-Köpfe, Track-Zeilen) bekommen die `v-wave`-Welle; **Buttons** bekommen das Press-Feedback ihrer Rolle und **keine** Welle — sonst kämpfen zwei Effekte um dasselbe Element. Farbe und Dauer stehen **einmal** in `main.ts` (`var(--mem-line)`, 0,35 s), nicht pro Aufrufstelle. v-wave setzt `overflow: hidden` übrigens auf einen **inneren** Container und erbt dessen Radius vom Wirt — Zeilenradius und absolut positionierte Drop-Marken bleiben also heil.
- **⚠️ JWT selbst prägen: `sub` muss ein Dict sein, kein JSON-String.** Der `user_lookup_loader` macht `jwt_data["sub"]["id"]` — ein `json.dumps({"id": 1})` als `sub` führt zu **HTTP 500 auf jedem Endpoint**, während die App-Shell weiter rendert. Symptom: Headless-Session zeigt Chrome und Nav, aber **null Playlists, null Alben** — sieht nach kaputtem UI aus, ist aber ein kaputtes Token. Prägen mit PyJWT: `pyjwt.encode({"sub": {"id": 1}, "iat": …, "nbf": …, "exp": …, "type": "access", "fresh": False, "jti": …}, cfg["serverId"], algorithm="HS256")`, Secret aus `~/.config/swingmusic/settings.json`. Vor jedem Messlauf einmal `curl -H "Cookie: access_token_cookie=$T" localhost:1970/auth/user` prüfen.
- **⚠️ Headless: außerhalb des Viewports gibt es kein `elementFromPoint`.** Ein `page.mouse.down()` auf den Koordinaten eines Elements, das unterhalb der Falz liegt, landet auf **nichts** — der Test meldet „kein Effekt", der Code ist aber in Ordnung (real passiert: Ordner-Kopf bei y≈1044 in einem 900-px-Viewport). Vor jeder Zeiger-Interaktion `scrollIntoViewIfNeeded()` und danach mit `elementFromPoint` prüfen, dass die Mitte wirklich das Ziel trifft.
...
- **⚠️ `~/preview` gehört niemandem — für einen Messlauf einen eigenen Checkout nehmen.** Das Verzeichnis wird von mehreren Sitzungen benutzt: eine andere kann es mitten im Lauf auf ihren Branch zurücksetzen und neu bauen, und dann misst man deren Code (real passiert: erste Messung gelb, zweite gegen dasselbe `dist` wieder teal — `git log` in `~/preview` stand auf einem fremden Commit). Eigenen Baum anlegen: `git clone -q --shared ~/preview ~/preview-<thema>`, `origin` auf das Fork-Repo zeigen, `git fetch origin <branch> && git checkout FETCH_HEAD`, `ln -s ~/preview/node_modules node_modules`, dann `yarn build`. Danach **am gebauten Artefakt** prüfen, dass die eigene Änderung wirklich drin ist — `grep -o "<selektor>{[^}]*}" dist/assets/index.*.css` gegen die CSS, die `dist/index.html` auch tatsächlich lädt (es liegen mehrere `index.*.css` herum).
